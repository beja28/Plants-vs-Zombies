...
import tp1.p2.control.commands.FireWaveCommand;
...
public abstract class Command {
	...
	public static Command parse(String[] commandWords) throws GameException{
		...
		if (commandWords.length == 1 && commandWords[0].isEmpty()) {
			returnedCommand = Command.defaultCommand;
		}
		else {
			for (Command command : AVAILABLE_COMMANDS) {
				if (command.matchCommand(commandWords[0])) {
					String[] parametros = new String[commandWords.length-1];
					for(int i = 0; i < commandWords.length-1; i++) {
						parametros[i] = commandWords[i+1];
					}
					returnedCommand = command.create(parametros);
				}	
			}
